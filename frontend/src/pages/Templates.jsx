...
export default function SmoothEliteGallery() {
  ...
  useEffect(() => {
    const fetchTemplates = async () => {
      try {
        ...
        // Fallback chain to handle different backend response structures
        const data = res.data?.websites || res.data?.data || res.data || [];
        setTemplates(data);
        console.log("DATA:", res.data);
      } catch (error) {
        console.error("Error fetching websites:", error);
        setTemplates([]); // Prevent crash on error
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white selection:bg-orange-500 font-sans antialiased pb-20">
      ...
      {/* --- ELITE SPLIT CONTROL DOCK --- */}
      <nav className="top-0 z-40 flex justify-center mb-12 px-7">
        <motion.div 
          className="flex flex-row items-center mt-20 justify-between bg-white/[0.03] border border-white/10 p-1.5 rounded-[32px] backdrop-blur-2xl shadow-2xl w-full max-w-5xl group focus-within:border-orange-500/30 transition-all duration-500"
        >
          {/* LEFT: Search */}
          <div className="relative flex items-center max-w-xs w-full ml-2">
            <Search className="absolute left-3 text-gray-500 group-focus-within:text-orange-500 transition-colors" size={16} />
            <input 
              type="text"
              placeholder="Search library..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-transparent py-3 pl-10 pr-4 focus:outline-none text-sm font-medium placeholder:text-gray-600 tracking-tight"
            />
          </div>

          {/* RIGHT: Filter */}
          <div className="flex items-center gap-1 bg-black/40 rounded-[26px] p-1 border border-white/5">
            {['all', 'free', 'exclusive'].map((filter) => (
              <button
                key={filter}
                onClick={() => setActiveFilter(filter)}
                className={`relative px-6 py-2 text-[10px] font-black uppercase tracking-[0.15em] transition-all z-10 ${
          ...
          <motion.div layout className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <AnimatePresence mode='popLayout'>
              {filteredItems.map((item) => {
                const itemId = item._id || item.id; // Support both MongoDB and standard IDs
                return (
                  <motion.div
                    layout
        ...
        {selectedId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              transition={transition}
              className="relative w-full max-w-6xl bg-[#0F0F0F] rounded-[50px] border border-white/10 shadow-2xl overflow-hidden grid grid-cols-1 lg:grid-cols-2 h-fit max-h-[90vh] z-50"
            >
              <button 
                onClick={(e) => { e.stopPropagation(); setSelectedId(null); }}
                className="absolute top-8 right-8 z-[100] p-4 bg-white/10 text-white hover:bg-orange-500 rounded-full transition-all active:scale-90"
              >
                <X size={24} />
              </button>
              ...
              <div className="p-12 md:p-16 flex flex-col justify-center">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                  <div className="flex items-center gap-2 text-orange-500 mb-6 font-black uppercase tracking-[0.4em] text-[10px]">
                    <Zap size={14} fill="currentColor" /> {selectedId.type || 'Premium'} Asset
                  </div>
                  ...
                  <motion.div layoutId={`price-${selectedId._id || selectedId.id}`} transition={transition} className="text-3xl font-light italic text-gray-500 mb-8">
                    {selectedId.price}
                  </motion.div>
                  <div className="flex gap-4">
                    <button className="flex-1 bg-white text-black py-5 rounded-[24px] font-black text-xl hover:bg-orange-400 transition-all active:scale-[0.98]">
                      Deploy Now
                    </button>
                    <button className="w-20 bg-white/5 border border-white/10 rounded-[24px] flex items-center justify-center text-white hover:bg-white/10 transition-colors">
