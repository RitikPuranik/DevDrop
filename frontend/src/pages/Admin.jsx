    ...
    return (
      <div className="min-h-screen bg-[#080808] text-white flex items-center justify-center px-6">
        <div className="max-w-lg w-full rounded-[32px] border border-white/10 bg-white/[0.03] p-8 text-center backdrop-blur-2xl">
          ...
          <p className="text-white/40 text-sm leading-relaxed mb-8">This dashboard is locked to admin accounts. Sign in with an admin user to access the review queue.</p>
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="rounded-[32px] border border-white/10 bg-[#0f0f0f] p-6 md:p-8 shadow-2xl shadow-black/30">
          ...
          <div className="space-y-4 max-h-[760px] overflow-auto pr-2 custom-scrollbar">
            {pendingLoading ? (
              <div className="py-20 flex items-center justify-center text-white/30"><Loader2 className="animate-spin" size={32} /></div>
            ) : pending.length === 0 ? (
                    ...
                    {expandedId === item._id && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                        <div className="pt-6 space-y-4">
                          <Field label="Creator">
                            <div className="text-sm text-white/75 px-1">{item.sellerId?.name || item.sellerId?.email || 'Seller account locked for this request'}</div>
                          </Field>
                          <Field label="Seller GitHub Repo">
                            <div className="text-sm text-white/75 px-1 break-all">{pendingForm.githubUrl || 'No GitHub repo submitted by seller'}</div>
                          </Field>
                          ...
                          <Field label="Rejection Reason">
                            <textarea rows={2} value={pendingForm.rejectionReason} onChange={(e) => setPendingForm({ ...pendingForm, rejectionReason: e.target.value })} className="w-full bg-transparent outline-none resize-none" placeholder="Reason for rejection (if rejecting)..." />
                          </Field>
                          ...
                          <div className="grid md:grid-cols-2 gap-4 pt-2">
                            <button onClick={() => handleApprovePending(item._id)} className="w-full rounded-2xl bg-emerald-500 hover:bg-emerald-400 text-black py-4 font-black uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(16,185,129,0.15)] hover:shadow-[0_0_25px_rgba(16,185,129,0.25)]">
                              <CheckCircle size={18} /> Approve and list
                            </button>
                            <button onClick={() => handleRejectPending(item._id)} className="w-full rounded-2xl bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 py-4 font-black uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-2 transition-all">
                              Reject request
                            </button>
                          </div>
                        </div>
...
function UploadField({ label, accept, onChange, required = false }) {
  return (
    <label className="block rounded-3xl border border-dashed border-white/15 bg-white/[0.02] px-5 py-5 hover:border-[#8b7355]/70 transition-colors cursor-pointer group">
      <span className="flex items-center gap-2 text-[10px] uppercase tracking-[0.25em] text-white/50 font-bold mb-3 group-hover:text-[#8b7355] transition-colors"><Upload size={14} /> {label} {required && <span className="text-red-400">*</span>}</span>
      <input type="file" accept={accept} required={required} onChange={(e) => onChange(e.target.files?.[0] || null)} className="block w-full text-sm text-white/40 file:mr-4 file:rounded-full file:border-0 file:bg-white/10 hover:file:bg-[#8b7355] file:px-5 file:py-2.5 file:text-[10px] file:font-bold file:uppercase file:tracking-[0.2em] file:text-white file:transition-colors cursor-pointer" />
    </label>
  );
}
