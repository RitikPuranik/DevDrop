...
import DashboardShell, { DashboardLoading } from '../../components/account/DashboardShell';
...
export default function Dashboard() {
  ...
  if (loading) return <div style={cssVars}><DashboardLoading /></div>;
  ...
  const SECTION_META = {
    overview: { title: `Welcome back, ${profile?.name?.split(' ')[0] || 'there'}`, subtitle: "Here's what's happening across your account." },
    ...
    deployments: { title: 'Deployments', subtitle: 'Connected providers and live deployment history.' },
            ...
            <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                {stats.map((s, i) => (
                  <motion.div
                    ...
                    className="bg-[#121110] border border-white/8 rounded-3xl p-6 group hover:border-white/20 hover:bg-[#151413] transition-all shadow-lg shadow-black/20"
                  >
                    <div className="w-10 h-10 rounded-2xl flex items-center justify-center mb-4 transition-transform group-hover:scale-105" style={{ backgroundColor: `${s.color}1a` }}>
                      <s.icon size={18} style={{ color: s.color }} />
                    </div>
                    <p className="text-3xl font-black tracking-tight text-white">{s.value}</p>
                    <p className="text-[10px] uppercase tracking-[0.25em] text-white/60 mt-2 font-bold">{s.label}</p>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                <button onClick={() => setActiveSection('listings')} className="text-left bg-[#121110] border border-white/8 rounded-3xl p-6 hover:border-[var(--accent)]/40 transition-all group shadow-lg shadow-black/20">
                  ...
                  <p className="font-bold text-white text-sm mb-1">Manage Listings</p>
                  <p className="text-white/50 text-xs leading-relaxed">List a new project or edit an existing one.</p>
                </button>
                <button onClick={() => setActiveSection('deployments')} className="text-left bg-[#121110] border border-white/8 rounded-3xl p-6 hover:border-[var(--accent)]/40 transition-all group shadow-lg shadow-black/20">
                  ...
                  <p className="font-bold text-white text-sm mb-1">Deploy a Project</p>
                  <p className="text-white/50 text-xs leading-relaxed">Connect GitHub, Vercel, or Render and go live.</p>
                </button>
                <button onClick={() => navigate('/template')} className="text-left bg-[#121110] border border-white/8 rounded-3xl p-6 hover:border-[var(--accent)]/40 transition-all group shadow-lg shadow-black/20">
                  ...
                  <p className="font-bold text-white text-sm mb-1">Browse Marketplace</p>
                  <p className="text-white/50 text-xs leading-relaxed">Discover new templates from other sellers.</p>
                </button>
              </div>
            </motion.div>
          )}
          ...
          {activeSection === 'deployments' && (
            <motion.div key="deployments" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <div className="flex items-center justify-between mb-5">
                <p className="text-[10px] uppercase tracking-[0.3em] text-white/68 font-bold">
