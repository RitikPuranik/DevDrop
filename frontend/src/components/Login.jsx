...
import { Mail, Lock, User, X, Github, Chrome, Linkedin, ArrowRight } from 'lucide-react';
...
export default function AuthModal({ isOpen, onClose }) {
  ...
  return (
    ...
    >
      ...
      <div className={`relative overflow-hidden w-[800px] max-w-full min-h-[550px] bg-[#F5F2ED] rounded-[3rem] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] transition-all duration-700 ease-out transform ${
        isOpen ? "scale-100 translate-y-0" : "scale-95 translate-y-8"
      }`}>
        
        {/* Close Button */}
        <button onClick={onClose} className="absolute top-6 right-6 z-[110] p-2 hover:bg-[#EAE3D8] rounded-full text-[#8b7355] transition-all hover:rotate-90">
          <X size={24} />
        </button>
        ...
        <div className={`absolute top-0 h-full w-1/2 transition-all duration-700 ease-in-out left-0 opacity-0 z-[1] 
          ...
          <form className="flex flex-col items-center justify-center h-full px-12 text-center" onSubmit={e => e.preventDefault()}>
            <h2 className="text-4xl font-serif font-bold text-[#3d342b] mb-2">Create Account</h2>
            <p className="text-[#8b7355] mb-6 text-sm">Join our exclusive community</p>
        ...
        <div className={`absolute top-0 h-full w-1/2 transition-all duration-700 ease-in-out left-0 z-[2] 
          ...
          <form className="flex flex-col items-center justify-center h-full px-12 text-center" onSubmit={e => e.preventDefault()}>
            <h2 className="text-4xl font-serif font-bold text-[#3d342b] mb-2">Welcome</h2>
            <p className="text-[#8b7355] mb-6 text-sm">Please enter your credentials</p>
        ...
        <div className={`absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-all duration-700 ease-in-out z-[100] 
          ...
          <div className={`relative -left-full h-full w-[200%] bg-[#8b7355] transition-all duration-700 ease-in-out
            ...
            <div className="flex h-full w-full text-[#F5F2ED]">
              ...
              <div className="flex flex-col items-center justify-center w-1/2 px-12 text-center">
                ...
                <p className="text-white/80 text-sm mb-10 leading-relaxed font-light">If you already have an account, just sign in. We've missed you!</p>
                <button onClick={() => setIsSignUp(false)} className="px-12 py-3 border border-white/40 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-[#F5F2ED] hover:text-[#8b7355] transition-all">
                  Sign In
                </button>
              </div>
              ...
              <div className="flex flex-col items-center justify-center w-1/2 px-12 text-center">
                ...
                <p className="text-white/80 text-sm mb-10 leading-relaxed font-light">Start your journey with us today and access exclusive features.</p>
                <button onClick={() => setIsSignUp(true)} className="px-12 py-3 border border-white/40 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-[#F5F2ED] hover:text-[#8b7355] transition-all">
                  Sign Up
                </button>
              </div>
            </div>
...
const AuthInput = ({ icon: Icon, ...props }) => (
  <div className="relative w-full mb-4 group">
    <Icon className="absolute left-4 top-1/2 -translate-y-1/2 text-[#8b7355]/50 group-focus-within:text-[#8b7355] transition-colors" size={18} />
    <input 
      {...props} 
