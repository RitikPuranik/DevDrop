  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#f1ece2] antialiased">
      <div className="flex">
        ...
        {/* ── MOBILE TOPBAR (dashboard section menu) — sits below the site navbar ── */}
        <div className="lg:hidden fixed top-[78px] left-0 right-0 z-40 flex items-center justify-between px-4 py-3 bg-[#0a0a0a]/95 backdrop-blur-md border-b border-white/8">
          <p className="text-[10px] uppercase tracking-[0.25em] text-white/50 font-bold">Dashboard</p>
          <button
          ...
          <div className="max-w-6xl mx-auto px-5 sm:px-8 py-8 lg:py-12">
            {(pageTitle || headerAction) && (
              <div className="flex items-start justify-between gap-4 mb-8">
                <div>
                  {activeItem && (
                    <p className="flex items-center gap-1.5 text-[10px] uppercase tracking-[0.25em] text-white/40 font-bold mb-2">
                      Dashboard <ChevronRight size={11} /> <span className="text-[var(--accent)]">{activeItem.label}</span>
                    </p>
                  )}
                  <h1 className="text-2xl md:text-3xl font-black tracking-tight text-white">{pageTitle}</h1>
                  {pageSubtitle && <p className="text-white/55 text-sm mt-1.5">{pageSubtitle}</p>}
                </div>
                {headerAction}
              </div>
            )}
...
function SidebarInner({ profile, navItems, activeSection, onSelectSection, onLogout, navigate }) {
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="hidden lg:block h-24 shrink-0" />
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3.5 py-2.75 rounded-xl text-sm font-semibold transition-all group relative ${
            ...
            >
              ...
              <item.icon size={16} className={isActive ? 'text-[var(--accent)]' : 'text-white/40 group-hover:text-white/70'} />
      ...
      <div className="px-3 pb-5 pt-3 mt-2 border-t border-white/8 space-y-1">
        <button
          onClick={() => navigate('/profile')}
          className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold text-white/55 hover:text-white hover:bg-white/[0.04] transition-all"
        >
        ...
        <button
          onClick={onLogout}
          className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold text-white/55 hover:text-red-300 hover:bg-red-500/10 transition-all"
        >
